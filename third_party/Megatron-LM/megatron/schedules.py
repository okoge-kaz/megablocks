...
def forward_step(forward_step_func,
    ...
    unwrapped_model = unwrap_model(
        model, (torchDDP, LocalDDP, Float16Module))  # type: ignore
    ...
    unwrapped_model.set_input_tensor(input_tensor)  # type: ignore
    ...
    with context_handler():
        for i in range(get_num_microbatches() - 1):  # type: ignore
            output_tensor = forward_step(
                ...
                is_eval=forward_only,  # forward only = eval
            )
            if not forward_only:
                backward_step(
                    optimizer,
...
def forward_backward_pipelining_with_interleaving(forward_step_func,
    ...
    num_microbatches = get_num_microbatches() * num_model_chunks  # type: ignore
    ...
    def backward_step_helper(microbatch_id):
        ...
        if mpu.is_pipeline_last_stage():
            if len(output_tensor_grads[model_chunk_id]) == 0:  # type: ignore
                output_tensor_grads[model_chunk_id].append(None)  # type: ignore
        input_tensor = input_tensors[model_chunk_id].pop(0)
        output_tensor = output_tensors[model_chunk_id].pop(0)
        output_tensor_grad = output_tensor_grads[model_chunk_id].pop(0)  # type: ignore
        input_tensor_grad = \
        ...
        if k == (num_warmup_microbatches - 1) and not forward_only and \
            ...
            input_tensor, output_tensor_grad = \
                p2p_communication.send_forward_backward_recv_forward_backward(
                    output_tensor_to_send,
                    input_tensor_grad,
                    recv_prev=recv_prev, recv_next=recv_next,
                    tensor_shape=tensor_shape,
                    timers=timers
                )
            output_tensor_grads[num_model_chunks - 1].append(output_tensor_grad)  # type: ignore
        else:
            input_tensor = \
    ...
    for k in range(num_microbatches_remaining):
        ...
        if recv_prev:
            input_tensors[next_forward_model_chunk_id].append(input_tensor)
        if recv_next:
            output_tensor_grads[next_backward_model_chunk_id].append(output_tensor_grad)  # type: ignore

    # Run cooldown backward passes (flush out pipeline).
    if not forward_only:
        if all_warmup_microbatches:
            output_tensor_grads[num_model_chunks - 1].append(  # type: ignore
        ...
        for k in range(num_microbatches_remaining, num_microbatches):
            ...
            if k == (num_microbatches - 1):
                recv_next = False
            output_tensor_grads[next_backward_model_chunk_id].append(  # type: ignore
...
def forward_backward_pipelining_without_interleaving(forward_step_func,
    ...
    num_warmup_microbatches = min(
        num_warmup_microbatches,
        num_microbatches)  # type: ignore
    num_microbatches_remaining = \
        num_microbatches - num_warmup_microbatches

    unwrapped_model = unwrap_model(
        model, (torchDDP, LocalDDP, Float16Module))  # type: ignore
    model_type = unwrapped_model.model_type  # type: ignore
